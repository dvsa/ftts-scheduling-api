  ...
  describe('makeReservation', () => {
    describe('succeeds', () => {
      ...
      test('given multiple valid reservations', async () => {
        ...
        (mockContext.req.body as ReservationsRequest[]).push(secondValidReservationsRequest);
  ...
  describe('deleteReservation', () => {
    ...
    describe('errors', () => {
      ...
      test('given an invalid reservation id in params', async () => {
        mockContext.req.params.reservationId = 123 as unknown as string;

        await expect(ReservationsController.deleteReservation(mockContext)).rejects.toThrow(SchedulingError);
        expect(logger.warn).toHaveBeenCalledWith('RESERVATIONS_CONTROLLER::validateReservationId: Reservation ID incorrect length or not a string', { reservationId: mockContext.req.params.reservationId });
      });
    });
  });
});
